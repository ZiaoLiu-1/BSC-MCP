...
class GMGNClient:
    ...
    def fetch_token_trades(self, token_address, from_timestamp=0, to_timestamp=int(time.time()), limit=100, maker="", next_page=None):
        ...
        """
        base_url = "https://gmgn.ai/api/v1/token_trades/bsc"
    ...
    def get_top_traders(self, token_address, limit=100, orderby="profit", direction="desc", save_to_file=False):
        ...
        """
        base_url = "https://gmgn.ai/defi/quotation/v1/tokens/top_traders/bsc"
        ...
        try:
            ...
            if save_to_file:
                with open('top_traders.txt', 'w') as f:
                    for trader in trader_details:
                        f.write(f"{trader['address']}\n")
                print(f"Successfully saved {len(trader_details)} trader addresses to top_traders.txt", file=sys.stderr)
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='GMGN API Client')
